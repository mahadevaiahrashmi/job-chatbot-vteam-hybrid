...
_PAGE_SIZE = 20
_USER_AGENT = "job-chatbot-vteam-hybrid/0.1 (+https://github.com/mahadevaiahrashmi/job-chatbot-vteam-hybrid)"
_JOB_ID_RE = re.compile(r"_([A-Z0-9-]+WD)(?:-\d+)?$")


def _extract_job_id(external_path: str) -> str:
    """Pull the canonical job ID (e.g. ``712616WD``) out of an externalPath.

    Workday externalPath looks like::

        /Global_Experienced_Careers/job/Bengaluru-Millenia/IN-Senior-..._712616WD

    or sometimes with a ``-1`` / ``-2`` suffix. We strip the suffix so the
    same role surfaced on two careers sites still deduplicates.
    """
    match = _JOB_ID_RE.search(external_path or "")
    if match:
        return match.group(1)
    # Fall back to the last path segment if we can't parse a clean ID.
...
def search_jobs(
    ...
    """Search a Workday tenant for postings matching ``keywords``/``location``."""
